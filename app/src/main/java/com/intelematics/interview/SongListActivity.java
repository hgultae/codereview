...
import com.intelematics.interview.adapters.SongsListAdapter;
import com.intelematics.interview.db.DBManager;
import com.intelematics.interview.models.Song;
import com.intelematics.interview.net.IWebService;
import com.intelematics.interview.net.RetrofitManager;
...
public class SongListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        mRecyclerSongs = (RecyclerView) findViewById(R.id.recycler_songs);
        mRecyclerSongs.setLayoutManager(new LinearLayoutManager(this));
	...
	public void retrieveSongList(){
        ...
        RetrofitManager retrofitManager = new RetrofitManager(Utils.BASE_URL);
        IWebService mWebService = retrofitManager.provideRetrofit().create(IWebService.class);
        Call<Song> call = mWebService.getSongs();
        call.enqueue(new Callback<Song>() {
            @Override
            public void onResponse(Call<Song> call, Response<Song> response) {

                loadDialog.dismiss();
                Log.i(TAG, "size: " + response.body().getResults().size());

                //SongManager songManager = new SongManager(SongListActivity.this, dbManager);
                //songManager.saveSongsList(response.body());

                mSongsListAdapter = new SongsListAdapter(SongListActivity.this, response.body(), R.layout.song_list_row);
                mRecyclerSongs.setAdapter(mSongsListAdapter);
